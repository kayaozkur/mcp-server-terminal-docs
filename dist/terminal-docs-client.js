...
export class TerminalDocsClient {
    ...
    async regenerateCommandDatabase() {
        try {
    ...
    async getCommandsByType(toolType) {
        const commands = [];
        for (const [name, cmd] of this.commandDatabase) {
    ...
    getStats() {
        ...
        const toolTypes = [...new Set(Array.from(this.commandDatabase.values()).map(c => c.tool_type).filter(t => t !== undefined))];
...
//# sourceMappingURL=terminal-docs-client.js.map