  ...
  private generateContextualPlaceholders(originalPrompt: string): { name: string; target: string; description: string }[] {
    ...
    if (prompt.includes("file") || prompt.includes("directory")) {
      placeholders.push(
        ...
        { name: "backup_location", target: "backup", description: "Backup directory location" }
      );
    }

    if (prompt.includes("api") || prompt.includes("endpoint")) {
      placeholders.push(
        { name: "api_endpoint", target: "endpoint", description: "API endpoint URL" },
        { name: "auth_method", target: "authentication", description: "Authentication method" }
      );
    }

    if (prompt.includes("database") || prompt.includes("data")) {
      placeholders.push(
        { name: "db_connection", target: "database", description: "Database connection string" },
        { name: "table_name", target: "table", description: "Target table name" }
      );
    }
